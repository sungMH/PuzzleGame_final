...
//퍼즐이 완성 되었는지 체크하는 함수
//매개변수: 퍼즐 한 변의 길이, 게임 보드, 초기 보드, 시작 시각, 걸린 시간 저장할 변수 주소값
void check(int size, ObjectPtr game_board[], ObjectPtr init_board[], time_t start_time, time_t * time_game) {
	for (int k = 0; k < size*size; k++) {
		...
		if (k == size*size-1) {
			...
			*time_game = difftime(end_time, start_time);
			string buf = to_string(*time_game) + "초 걸렸습니다!";
	...
	for (int i = 0; i < 16; i++) {
		...
		game_board1[i]->setOnMouseCallback([&](ObjectPtr object, int, int, MouseAction)->bool {
			...
			if (movable(j,blank1,size1)) {
				...
				check(size1, game_board1, init_board1, start_time, &time_game1);
			}
			return true;
			});	
	}
	...
	for (int i = 0; i < 9; i++) {
		...
		game_board2[i]->setOnMouseCallback([&](ObjectPtr object, int, int, MouseAction)->bool {
			...
			if (movable(j, blank2, size2)) {
				...
				check(size2, game_board2, init_board2, start_time, &time_game2);
			}
			return true;
			});
	}
